...
class TestHrrrTotalPrecip20220818f06(unittest.TestCase):
    """HRRR Total Precip; File from 2022-08-18; Forecast Hour 06"""

    def setUp(self) -> None:
        self.acquirable = os.path.join(
            os.path.dirname(__file__), "..", "fixtures", "hrrr-total-precip", "hrrr-total-precip-20220818", "hrrr.t00z.wrfsfcf06.grib2"
        )
        ...
        if not os.path.isfile(self.acquirable):
            ...
            urlretrieve(
                'https://noaa-hrrr-bdp-pds.s3.amazonaws.com/hrrr.20220817/conus/hrrr.t00z.wrfsfcf06.grib2',
                self.acquirable
            )
    ...
    def test_find_correct_band(self) -> None:
        ...
        ds = None
        # Get the band; 90 is the band we want in the sample file
        self.assertEqual(band, 90)
